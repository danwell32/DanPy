...
def plot_randomly_spectra(samples,k,e_init,e_end,dig_spectra=None,fig_save=None):
    '''
    ...
    e_resol=(e_end-e_init)/n_channels
    ...
    if dig_spectra is not None: 
        for i, spec in enumerate(dig_spectra):
            plt.plot(np.arange(e_init,e_end,step=e_resol),spec,color='black',linewidth=0.7,linestyle='-')
        
    lenght = len(samples)
    for i in range(0,k):
        spectro_ind = randrange(0,lenght)
        plt.plot(np.arange(e_init,e_end,step=e_resol),samples[spectro_ind],color=comap(100-i*12),linewidth=0.7,linestyle='-')
        
    plt.xlabel('Energy Loss (eV)')
    plt.ylabel('Normalized Intensity')
    
    #plt.vlines(x=710.,ymin=-0.1,ymax=1.1,linestyles=':',linewidth=2,color='red')
    plt.gca().set(xlim=(e_init,e_end),ylim=(-0.01,1.01))
    plt.xticks(np.arange(e_init,e_end,step=10))
    plt.yticks(np.arange(0,.3, step=0.05))

...
def cross_fold_validation(model,x_val,y_val,x_test,y_test,train_size,shift=False,val_shift=0.,cv=10):
    ...
    if shift: #Chemical shift:
        for i in range(0,len(train[:])):
            ...
            scores.append(model.score(test[i][0][:50+val_shift:600+val_shift],test[i][1])) #Ojo aqyí hay q revisar el shift. 
...
def noise_PCA_spectra(model, samples, noisy_samples, labels, noise_mult=5,criteria=0.01,scree_plot=False):
    '''
    ...
    -scree_plot: boolean. (default = False) 
    	If True returns the scree plot of the PCA routine.
    -criteria: float. (default = 0.01)
        Value to select the low variance components. 
    ...
    spectra_train = np.copy(samples)
    spectra = np.copy(noise_samples)
    ...
    nComp = 0
    while noise_model.explained_variance_[nComp]>criteria:
        nComp += 1
    ...
    num = int((noise_mult-1)*2)#*10) #Quantity of noise multiples from 0 to noise_mult.
    print('Number of mulples of noise to evaluate the model:',num,'.')
    SNR = np.linspace(1,noise_mult,num) #linear vector with the noise multiples. 
    ...
    for i in range(len(SNR)):
        ...
        for spectrum in noise_test[i]:
            ...
            j = j+1
        noise_acc.append(cross_fold_validation(model,spectra_train,labels,noise_test[i].reshape(samples.shape[0],samples.shape[1]),labels,.65,False)) 
...
def chemical_shifts(model,x_val,y_val,x_test,y_test,num_bins_translate,train_size,cv=10):
    ...
    print('Número de bins para calcular la traslación:',int((num_bins_translate/2)/5),'.')

    for i in range(int((num_bins_translate/2)/5)):    #Divido entre 5 porque sino se va de madre el calculo. 
        i=i*5
        accuracy_shift_plus.append(cross_fold_validation(model,x_val,y_val,x_test,y_test,train_size,True,val_shift=i,cv=cv))
    for j in range(int((num_bins_translate/2)/5)):
        j=j*5
